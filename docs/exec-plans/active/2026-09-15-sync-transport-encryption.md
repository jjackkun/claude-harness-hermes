...
- [ ] 목표 9 — 기억 "없음" 3분류(H-11)를 기계가 낸다. 검증: 저장소 없음 / 조각 미복호 / 조회 0건 세 경우 각각 다른 문구.
...
- 명령(각 원격에서, AI 세션 밖 터미널):
  ...
  ```
- 산출: 결과를 §7 에 기록. 둘 중 하나라도 거부되면 §6 의 폴백(브랜치 `hermes/sync`)으로 전환하고 `hermes_sync_ref.py` 의 참조 이름만 바꾼다.
- 검증: 목표 13.
...
- (Step 0 결과 기록 자리) V-1 gitlab.com: — / V-2 사내 서버 종류·push: — / V-3 광고: —
