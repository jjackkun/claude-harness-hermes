...
> **9/29 에 함께 볼 항목**: `docs/exec-plans/backlog/agent-model-routing-blindspot.md`
> (에이전트 `model:` 선언의 타당성을 잴 데이터가 없음). 목표 5의 답이
> "계장이 값을 한다" 를 지지할 때만 승격한다 — 부정이면 진행하지 않는다.
...
- **신규 파일 목록**:
  ...
  - `assets/hooks/out_report.py` — (2026-09-20 추가) R-out 기록을 명령 머리별로 모아 바이트 분포(n·p50·p90·max·임계 초과)를 낸다. 목표 5 의 "명령별 분포" 를 실제로 내는 도구.
...
## 7-4. 기록 형식 보강 (2026-09-20)

목표 5 의 "명령별 바이트 분포" 를 지금 기록으로는 못 낸다는 것을 확인했다 — R-out 레코드가 바이트·소요 시간만 남기고
어떤 명령이었는지는 비어 있었다(path 빈 칸). 9월 29일까지 기다려도 그 칸은 채워지지 않는다.

- 훅이 **명령 머리**를 path 칸에 `cmd:<머리>` 로 남긴다. 머리 = `cd X &&`·`VAR=값` 을 걷어낸 첫 토큰(+인터프리터면 둘째 토큰의 basename).
  값·옵션은 남기지 않는다(비밀값이 기록에 새지 않게 — 시험으로 단언). 명령 패턴 매칭은 여전히 0(목표 2 유지).
- `assets/hooks/out_report.py` 가 머리별 n·p50·p90·max·임계 초과·합계를 낸다. 머리 없는 옛 기록은 `(머리 없음)` 한 줄로 센다.
- 이 시점 실기록: 이 저장소 341건(p50 1,196B · p90 5,344B · max 30,002B · 초과 13), terminal-shipping 2,047건 — 전부 옛 형식.
  9월 29일 판정은 오늘 이후 쌓이는 새 형식 기록으로 한다.
