...
> 상태: **설계 단계(구현 전).** 큰 주제 4건 중 Q-1(에이전트 생성) · Q-2(인계 계약) · Q-3(기존 자산 정리) · Q-4(복사 설치 예외) **모두 완료.** 다음은 실측 항목(V-1~V-6) 확인과 실행 계획 작성. 남은 주제와 미확인 항목은 [open-questions.md](open-questions.md).
...
| 정체성 | id(불변) · 이름(a.k.a.) · 상태 · 조직 값 (직무·소속은 소우주 조직 정의의 값) | 확정 |
