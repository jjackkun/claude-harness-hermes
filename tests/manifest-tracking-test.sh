...
untracked_flag() { doctor_json "$1" | python3 -c "import json,sys;d=json.load(sys.stdin);print('yes' if d.get('manifest_untracked') else 'no')" 2>/dev/null || echo err; }
...
assert "무시되는 곳 → 추적되지 않음 표시" "yes" "$(untracked_flag "$T/ignored")"
...
assert "예외가 덮여도 잡는다" "yes" "$(untracked_flag "$T/overridden")"
# 자기 검사 — 순서를 되돌리면(예외가 뒤) 추적되므로 표시가 사라져야 한다
printf '.claude/*\n!.claude/.factory-manifest.json\n' > "$T/overridden/.gitignore"
assert "예외가 뒤에 오면 표시 없음(자기 검사)" "no" "$(untracked_flag "$T/overridden")"
