...
- **스킬** (아래 발동 조건에 걸리면 호출한다. 작업 크기와 무관하다): `.claude/skills/`
  ...
  - hermes-agent — 헤르메스 에이전트 명부·조직·소환·인계를 CLI 로 잇는다. `/hermes-agent` 라고만 치거나 "에이전트 만들자"·"한 명 입사시켜"·"직원 뽑자" 처럼 분야·직급·조직 없이 입사를 말하면 터미널…
  - hermes-dashboard — 소우주(현재 프로젝트)의 에이전트·스킬·학습 루프·건강 현황을 정적 HTML 한 장으로 만들어 경로를 알린다. 사용자가 `/hermes-dashboard` 를 치거나 "대시보드 보여줘", "현황 페이지", "에이전트…
...
- **에이전트** (검토·위임): `.claude/agents/` — architect-lite devil-advocate planner-lite architect planner code-reviewer silent-failure-hunter tdd-guide doc-updater docs-lookup performance-optimizer refactor-cleaner
...
- `~/.hermes/global.db` — 전역 SQLite (공통 패턴 + 사용자 성향: `hermes-persona.py distill` 로 쌓고 `review` 로 승인 → 세션 시작에 주입)
