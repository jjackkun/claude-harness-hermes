...
│   ├── hermes-cron-run.sh            ← cron 용 러너 (manager/dream 액션)
...
| python 스크립트 18개 | `[project]/scripts/hermes-*.py`, `hermes_redact.py`, `hermes_skills.py` + cron 러너 `hermes-cron-run.sh` |
...
[별도] hermes-dream.py (cron `dream` 액션) — 누적 5슬롯 요약을 읽어 반복된 결정/사실을
        결정화로 승격하고, junk 스킬 삭제를 '제안'(실행은 --apply 게이트)
```
