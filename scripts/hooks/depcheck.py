...
def _guards_import(node: ast.Try) -> bool:
    """핸들러가 ImportError 를 잡고 **fallback 을 실제로 두는가**.
...
def _optional_nodes(tree: ast.AST) -> set[int]:
    """try 본문 안의 import 중 선택적인 것들의 id 집합."""
    found: set[int] = set()
    for node in ast.walk(tree):
        if not isinstance(node, ast.Try) or not _guards_import(node):
            continue
...
def _forbid_violations(path, mod, target, forbids, optionals, is_optional):
    """R-dep-3 — 금지된 경계를 넘는가."""
    exempt = (path, mod) in optionals and is_optional
    out = []
    for src_pat, dst_pat in forbids:
        if not (fnmatch.fnmatch(path, src_pat) and fnmatch.fnmatch(target, dst_pat)):
            continue
        if exempt:
            continue
        extra = ""
...
def _tier_violation(path, mod, target, tiers):
    """R-dep-1 — 하위 계층이 상위를 import 하는가."""
...
def _unregistered_warning(path, scope, tiers):
    """R-dep-4 — 계약 범위 안인데 tier 가 없는가 (경고)."""
...
def _check(paths: list[str], contract: dict) -> tuple[list[str], list[str]]:
    ...
    for path in paths:
        ...
        if not imports and not os.path.exists(path):
            continue
        warnings.extend(_unregistered_warning(path, scope, tiers))
        for mod, is_optional in imports:
            ...
            violations.extend(
                _forbid_violations(path, mod, target, forbids, optionals, is_optional))
            violations.extend(_tier_violation(path, mod, target, tiers))
