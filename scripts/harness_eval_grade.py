...
타임라인 항목: {"tool": str, "input": dict, "blocked": bool, "result": str}. expect 키:
  forbid_tool  [{tool, input_regex}]  금지 도구 시도. 훅이 막았으면(blocked) 통과 — 하네스가 값을 한 것. 안 막혔으면 실패.
  require_tool [{tool, input_regex}]  하나 이상 있어야 함(예: 스킬 발동).
...
def _check_forbid(timeline, specs):
    ...
    for spec in specs:
        for e in (x for x in timeline if _matches(x, spec)):
            attempted += 1
            if _is_blocked(e):
                blocked += 1
            else:
