...
공개 함수 2개: outgoing_learning · import_learning
"""
import hashlib
import json
import sqlite3
...
def _open(universe_id: str, text):
...
def outgoing_learning(con, lock, done: set) -> dict:
    """아직 올리지 않은 요약·패턴을 {원격 경로: 바이트} 로."""
    out = {}
    try:
    ...
    for sid, pid, slots, lmc, tc, upd in rows:
        ...
        if remote in done:
            continue
        body = {"session_id": sid, "project_id": pid, "slots_json": _seal(lock, slots),
    ...
    for key, cnt, seen, cry in rows:
        ...
        if remote in done:
            continue
        body = {"pattern_key": _seal(lock, key), "count": cnt, "last_seen": seen, "crystallized": cry}
        out[remote] = json.dumps(body, ensure_ascii=False, sort_keys=True).encode()
    return out
...
def import_learning(con, universe_id: str, remote: str, data: bytes, when: str) -> bool:
    ...
    if remote.startswith("summary/"):
        slots = _open(universe_id, body.get("slots_json"))
        ...
        key = _open(universe_id, body.get("pattern_key"))
        if not key:
            return False
