...
사람이 목록을 적지 않는다 — 열쇠처럼 강제가 되면 켜지지 않는다(2026-09-20 논의). 컴퓨터가 이미 아는 것만 쓴다:
  · `git config user.name`  · 최근 커밋 작성자(200건)  · 명부(agents.json)의 이름  · OS 사용자명
...
def _roster_names(project_dir: str) -> set:
    path = os.path.join(project_dir, ".hermes", "agents.json")
    try:
        with open(path, encoding="utf-8") as fh:
            return {a.get("name", "") for a in json.load(fh).get("agents", [])}
...
def load_known_names(project_dir: str = None) -> tuple:
    ...
    names |= _roster_names(project_dir)
    try:
