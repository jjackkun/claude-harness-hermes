...
description: TypeScript/JavaScript reviewer for risky or high-impact changes involving security, async correctness, public APIs, shared types, state management, build config, or broad refactors. Skip for trivial local edits.
...
- **Multiple responsibilities per file**: 1 file = 1 responsibility named by its filename. ≥2 → split and re-export via barrel (`index.ts`)
- **File size > 500 lines (hard) / > 400 (soft warn)**: Almost always indicates mixed responsibilities; split by responsibility, not by line count
