...
do_uninstall() {
  ...
  if [[ -d "$scripts_dir" ]]; then
    ...
    mapfile -t hermes_scripts < <(find "$scripts_dir" -maxdepth 1 \( -name "hermes-*.py" -o -name "hermes-cron-run.sh" \) 2>/dev/null)
...
for path in "${TARGETS[@]}"; do
  ...
  hermes_py_count=$(find "$path/scripts" -maxdepth 1 \( -name "hermes-*.py" -o -name "hermes-cron-run.sh" \) 2>/dev/null | wc -l)
