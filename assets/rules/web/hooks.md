...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Write",
        "command": "node -e \"let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{const i=JSON.parse(d);const c=i.tool_input?.content||'';const lines=c.split('\\n').length;if(lines>500){console.error('[Hook] BLOCKED: File exceeds 500 lines ('+lines+' lines)');console.error('[Hook] 1 file = 1 responsibility — split and re-export via barrel');process.exit(2)}console.log(d)})\"",
        "description": "Block writes that exceed 500 lines (R-size hard cap)"
      }
    ]
  }
}
