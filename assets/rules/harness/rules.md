...
**반대편 한 명 더 (선택)** — architect-lite 리뷰 **뒤**, 또는 숫자로 결정을 닫을 때
"`devil-advocate` 를 붙일까요?" 라고 한 번 묻는다. 자동 실행 금지.
받은 반론은 **반증 조건을 직접 돌려** 성립/무너짐을 가른 뒤 보고한다(그대로 옮기지 않는다).
근거: 공장 저장소 `docs/audits/2026-09-21-devil-advocate-trial.md`.
