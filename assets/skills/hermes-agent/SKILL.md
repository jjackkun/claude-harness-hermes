...
- ①~③은 한 번의 `AskUserQuestion` 에 담는다(축이 셋 다 있으면 세 질문). 각 질문에 "없음" 선택지를 두고, 정의에 없는 값은 "Other" 로 받는다.
  선택지는 질문당 4개까지다 — 축 값이 더 많으면 [0-5](#0-5-후보가-4개를-넘을-때--숨기지-않는다) 대로 두 단계로 좁힌다.
...
- 초안 파일 경로와 **역할 문단 한 줄**만 보여주고 승인 여부를 묻는다. **승인은 사람의 말이 있을 때만** — 사용자가 "승인" 이라고 말한 턴에 `python3 scripts/hermes-agent.py approve-soul "<이름>"` 을 실행한다(표시 줄 삭제 + 이력 기록).
  말이 없는데 표시 줄을 지우려 하면 identity-guard 가 막는다. 소환된 에이전트는 어떤 경우에도 SOUL 을 승인하지 못한다. 내용 다듬기(표시 줄을 남기는 편집)는 자유다.
...
### 0-5. 후보가 4개를 넘을 때 — 숨기지 않는다

`AskUserQuestion` 은 질문 하나에 선택지를 **4개까지만** 받는다(뒤의 "Type something"·"Chat about this" 는 도구가 붙인다).
넘치는 후보를 선택지에서 그냥 빼면 사람은 **그런 값이 있다는 것조차 모른다** — 2026-09-22 템플릿 12개 중 3개만 보였고,
나머지 9개는 이름을 알아야 직접 입력할 수 있었다. 고르는 폼의 목적은 외우지 않게 하는 것이므로, 넘치면 **나눠서 보인다**.

| 후보 수 | 이렇게 묻는다 |
|---|---|
| 4 이하 | 선택지 그대로 |
| 5~16 | **두 단계** — ① 묶음 고르기 → ② 고른 묶음 안에서 고르기 |
| 17 이상 | 먼저 조건(분야 등)으로 거른 뒤 위 표를 다시 적용. 거를 조건이 없으면 본문 전체 나열 + 직접 입력 |

두 단계는 이렇게 만든다:

- **1단계 선택지 = 바로 고르는 값 1~2개 + 나머지 묶음.** 추천(요청 문장에 근거가 있는 값)이나 자주 쓰는 값을 그대로 두면
  그것이 맞을 때 한 번에 끝난다 — 질문을 한 번 더 띄우는 것은 그 값이 아닐 때뿐이다. 근거가 없으면 전부 묶음으로 둔다.
  바로 고르는 값은 묶음 안에 **다시 넣지 않는다** — 같은 이름이 두 곳에 보이면 어느 쪽을 눌러야 하는지 헷갈린다.
- **"없음" 도 한 칸이다.** 축 값 6개에 "없음" 을 더하면 7칸이 되어 넘친다 — 후보 수를 셀 때 "없음" 을 포함한다.
- **묶음마다 description 에 그 안의 이름을 전부 적는다.** 묶음 이름만 보고는 무엇이 들었는지 모른다.
- **묶음 하나는 4개 이하로 나눈다** — 2단계에서 또 넘치면 두 단계로 나눈 의미가 없다. 바로 고르는 값 1 + 묶음 3 이면 13개, 묶음 4 면 16개까지 된다.
- **묶음 기준은 후보가 이미 가진 속성에서 고른다** — 템플릿이면 역할 성격(예: 설계·계획 / 코드·문서 읽기 / 조사·전문)이나 `origin`,
  축 값이면 템플릿 출처(사무형·제품형). 기준을 지어내지 않고, 질문 본문에 기준 한 줄을 밝힌다.
- **질문 본문에는 단계와 무관하게 후보 전체 이름을 나열한다.** 묶음이 사람의 생각과 달라도 전체를 보고 Other 로 이름을 칠 수 있다.
- 2단계는 고른 묶음의 후보(4개 이하)를 선택지로 두고, 1단계에서 이미 답이 난 축은 다시 묻지 않는다.

0-2 처럼 한 폼에 여러 축을 담을 때는 **넘치는 축만** 두 단계로 뺀다 — 넘치지 않는 축은 첫 폼에서 함께 묻고,
넘치는 축의 2단계만 다음 폼으로 넘긴다. 폼을 연달아 띄우는 것 자체가 부담이므로 2단계 폼에는 그 축 하나만 둔다.

**예** — 기획 분야 템플릿 12개
→ 1단계: `product-manager (추천)` · `설계·계획: architect, code-architect, planner, gan-planner` ·
`코드·문서 읽기: code-explorer, spec-miner, doc-updater, docs-lookup` · `조사·전문: researcher, a11y-architect, seo-specialist`
(본문에 12개 전체 나열) → "코드·문서 읽기" 를 고르면 2단계에서 그 4개가 선택지가 된다.
...
분야 하나에도 후보가 12개쯤 나온다(기획 12 · 2026-09-22 실측) — 폼으로 물을 때는 [0-5](#0-5-후보가-4개를-넘을-때--숨기지-않는다) 대로 좁힌다.
