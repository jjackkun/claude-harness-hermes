...
### 0-4. SOUL 세 답 — 옮겨 적기까지만

입사가 성공하면 `정체성: .hermes/agents/<id>/` 가 출력된다. SOUL.md 는 여전히 **사람이 채운다**. 이 스킬은 세 답을 폼으로 받아
템플릿의 해당 절에 **사용자 문장 그대로** 옮겨 적는 데서 멈춘다 — 에이전트가 문장을 지어내거나 다듬지 않는다.

```text
🧭 SOUL 세 답   (.hermes/agents/<id>/SOUL.md — 사람 승인으로만 고친다)

① 역할   ▸ 이 에이전트가 맡는 일 한 문단        → "## 역할" 의 괄호 자리
② 금지   ▸ 하지 말아야 할 것 한 줄(여러 줄 가능) → "## 금지" 에 항목 추가
③ 말투   ▸ 답하는 투 한 줄                       → "## 원칙" 에 "- 말투: …" 항목 추가
```

- 세 질문을 한 번의 `AskUserQuestion` 에 담고, 각 질문에 "나중에 직접 쓴다" 선택지를 둔다. 그 답이면 그 절은 건드리지 않는다.
- 옮겨 적은 뒤 `git diff` 대신 바뀐 절만 보여주고, "SOUL 은 사람 승인으로만 고칩니다" 를 다시 알린다.

**예** — "/hermes-agent" (빈 조직인 이 저장소에서)
→ 조직 정의 폼(사무형/제품형/직접) → `organization.yaml` 기록 → 입사 폼(분야·직급·조직·이름) →
`python3 scripts/hermes-agent.py hire "게이트QA" --org "QA,담당,harness"` 를 보여주고 실행 → SOUL 세 답 옮겨 적기.
